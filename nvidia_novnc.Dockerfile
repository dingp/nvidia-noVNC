...
ARG CUDA_BASE=docker.io/nvidia/cuda:$CUDA_VERSION-base-ubuntu22.04
ARG BASE=$CUDA_BASE
FROM $BASE
...
RUN apt-get update && \
    ...
    apt update  && \
    apt-get install -y --no-install-recommends pkg-config libglvnd-dev && \
    apt-get -f -y install && \
    dpkg --configure -a && \
...
RUN apt-get update && \
    export DEBIAN_FRONTEND=noninteractive && \
    apt install -y software-properties-common && \
    apt-get install -y apt-utils net-tools software-properties-common && \
    dpkg --add-architecture i386 && \
    apt-get update -y && \
    apt-get install -y git wget curl net-tools qemu-kvm qemu nvidia-modprobe qemu-kvm \
        libvirt-clients libvirt-daemon-system bridge-utils virt-manager linux-tools-generic && \
    apt-get clean all && apt -y autoremove

RUN export DEBIAN_FRONTEND=noninteractive && \
    apt install -y software-properties-common && \
apt-get install -y  --install-recommends xrdp xfce4 xfce4-goodies \
       libsdl1.2-dev mesa-utils libgl1-mesa-dev libegl1-mesa-dev libgles2-mesa-dev && \
    apt-get clean all && \
    apt -y autoremove

RUN export DEBIAN_FRONTEND=noninteractive && \
    apt install -y software-properties-common && \
apt-get install -y x11vnc neofetch tightvncserver tigervnc-scraping-server novnc \
       xvfb xterm mc lynx open-vm-tools ssh-askpass-gnome && \
    apt-get clean all && apt -y autoremove
...
RUN export DEBIAN_FRONTEND=noninteractive && \
    apt install -y software-properties-common && \
apt-get -y -f install lm-sensors python3-pip clinfo freeglut3 && \
    apt-get clean all && \
    apt -y autoremove
